...
function getFileAndSizeAndHashForFile(file, fileGetter) {
    var buffer = fileGetter(file);
    return {
      file: file,
      size: stat.size,
      hash: getHash(buffer)
...
function getFilesAndSizesAndHashesForGlobPattern(globPattern, excludeFilePath, fileGetter) {
  return globPattern.forEach(file => getFileAndSizeAndHashForFile(file, fileGetter))
  ...
  return new Promise(function(resolve, reject) {
    ...
    params.staticFileGlobs.forEach(function(globPattern) {
      var filesAndSizesAndHashes = getFilesAndSizesAndHashesForGlobPattern(
        globPattern, params.outputFilePath, params.fileGetter);
    ...
    Object.keys(params.dynamicUrlToDependencies).forEach(function(dynamicUrl) {
      ...
      var filesAndSizesAndHashes = params.dynamicUrlToDependencies[dynamicUrl]
        ...
        .map(function(file) {
          try {
            return getFileAndSizeAndHashForFile(file, params.fileGetter);
          } catch (e) {
